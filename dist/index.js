...
//run();
const config = ['.gitignore,', 'asdf.md', '*.tftest.hcl', 'tests/**'];
const files = [
    ...
    ['tf-modules/s3-bucket-object', 'tf-modules/s3-bucket-object/README.md'],
    ['tf-modules/s3-bucket-object', 'tf-modules/s3-bucket-object/tests/a.tfatesta'],
    ['tf-modules/s3-bucket-object', 'tf-modules/s3-bucket-object/tests/sub/myfile'],
    ['tf-modules/s3-bucket-object', 'tf-modules/s3-bucket-object/variables.tf'],
    ['tf-modules/s3-bucket-object', 'tf-modules/s3-bucket-object/sub/asdf.md'],
    ['tf-modules/s3-bucket-object', 'tf-modules/s3-bucket-object/variables.tf'],
];
/**
 * Checks if a file should be excluded from matching a module change based on the
 * defined module change exclude patterns and relative paths from the base directory.
 *
 * @param {string} filePath - The path of the file to check.
 * @returns {boolean} True if the file should be excluded, false otherwise.
 */
function shouldExcludeModuleChangeFile(baseDirectory, filePath) {
    const relativePath = external_node_path_default().relative(baseDirectory, filePath);
    return config.some((pattern) => minimatch(relativePath, pattern, { matchBase: true }));
}
for (const file of files) {
    const match = shouldExcludeModuleChangeFile(file[0], file[1]);
