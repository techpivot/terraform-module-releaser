...
| `default-semver-level`              | The default semantic version level to use when a commit does not match any configured keywords. Valid values: `patch`, `minor`, `major`. Only applies when NO commits match any keywords. If any commit matches a keyword, that matched level takes precedence.                                                                                                                                                                                                                | `patch`                                                                                               |
...
| `wiki-content-source`               | Controls where wiki content comes from for each module. Valid values: `terraform-docs` (auto-generates with Usage, Attributes, Changelog), `readme` (uses module's README.md directly), or `readme-with-terraform-docs` (injects terraform-docs into README.md at marker position).<br><sub>[Read more here](#wiki-content-source-options)</sub>                                                                                                                               | `terraform-docs`                                                                                      |
| `wiki-include-changelog`            | Whether to include the changelog section in wiki pages. When `false`, the changelog section is omitted from wiki pages.                                                                                                                                                                                                                                                                                                                                                        | `true`                                                                                                |
| `wiki-readme-terraform-docs-marker` | Marker string in README.md where terraform-docs output should be injected. Only used when `wiki-content-source` is `readme-with-terraform-docs`. If marker not found, terraform-docs is appended at the end.                                                                                                                                                                                                                                                                   | `<!-- TERRAFORM_DOCS -->`                                                                             |
...
### Wiki Content Source Options

The `wiki-content-source` input controls how wiki pages are generated for each Terraform module. This allows you to
choose between auto-generated documentation or using your own README.md files.

#### Available Modes

| Mode                         | Description                                                                                                                                                                      |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `terraform-docs`             | **(Default)** Auto-generates wiki pages with Usage, Attributes (from terraform-docs), and Changelog sections. Best for modules without custom documentation.                     |
| `readme`                     | Uses the module's README.md file directly as wiki content. Changelog is appended if `wiki-include-changelog` is `true`. Ideal when you maintain detailed readme files.           |
| `readme-with-terraform-docs` | Injects terraform-docs output into your README.md at a marker position. Combines custom documentation with auto-generated Terraform documentation. Best of both worlds approach. |

#### Using `readme` Mode

When using `readme` mode, the action will use your module's README.md file as the wiki content:

```yaml
- uses: techpivot/terraform-module-releaser@v1
  with:
    wiki-content-source: readme
```

If a module doesn't have a README.md file, the action falls back to `terraform-docs` mode for that module.

#### Using `readme-with-terraform-docs` Mode

This mode allows you to maintain custom documentation while automatically injecting terraform-docs output. Add a marker
in your module's README.md where you want the terraform-docs to appear:

```markdown
# My Terraform Module

This module creates awesome infrastructure.
...
- uses: techpivot/terraform-module-releaser@v1
  with:
    wiki-content-source: readme-with-terraform-docs
    wiki-readme-terraform-docs-marker: "<!-- TERRAFORM_DOCS -->"
```

The marker will be replaced with the full terraform-docs output wrapped in `<!-- BEGIN_TF_DOCS -->` and
`<!-- END_TF_DOCS -->` tags. If the marker is not found in your README.md, terraform-docs output is appended at the end.

#### Controlling Changelog Inclusion

You can disable the changelog section in wiki pages using the `wiki-include-changelog` input:

```yaml
- uses: techpivot/terraform-module-releaser@v1
  with:
    wiki-content-source: readme
    wiki-include-changelog: false
```

When `wiki-include-changelog` is `false`, the Changelog section will be omitted from all wiki pages, regardless of the
content source mode.
